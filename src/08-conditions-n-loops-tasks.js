...
function isInsideCircle(circle, point) {
  const rad = Math.sqrt((circle.center.x - point.x) ** 2 + (circle.center.y - point.y) ** 2);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const fchar = isStartIncluded ? '[' : '(';
  const lchar = isEndIncluded ? ']' : ')';
  return `${fchar}${Math.min(a, b)}, ${Math.max(a, b)}${lchar}`;
}
 ...
 */
function isCreditCardNumber(cN) {
  const ccn = String(cN);
  let sum = 0;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let j = 0; j < pathes[0].length; j += 1) {
    for (let i = 1; i < pathes.length; i += 1) {
      if ((pathes[0][j] === '/') && (pathes[0][j] === pathes[i][j])) {
        lastSlash = j;
      }
      if (pathes[i][0] !== '/') {
        lastSlash = -1;
    ...
    if (found) {
      result = pathes[0].substring(0, lastSlash + 1);
      break;
    }
  }
  return result;
...
function getMatrixProduct(m1, m2) {
  const resMatrix = [];
  for (let i = 0; i < m1.length; i += 1) resMatrix[i] = [];
